...
@export var attackDecider_state: State  # State to transition to after the attack
@export var cleave_scene: PackedScene  # Packed scene for Cleave

var attack_timer: Timer = null

func enter() -> void:
	print("Cleave Right state entered")
	super()
...
func process_frame(delta: float) -> State:
	if attack_timer != null and not attack_timer.is_stopped():
		...
		return attackDecider_state  
	return null

func perform_cleave_attacks() -> void:
	perform_cleave(parent.global_position, 180.0)  
	
	# Second cleave (rotated 315 degrees from original position)
	var offset = Vector2(-100, -100)  # offset
	perform_cleave(parent.global_position + offset, 315.0)
